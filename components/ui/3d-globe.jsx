  ...
  return (
    <group ref={groupRef}>
      ...
      {markers.map((marker, index) => (
        <Marker key={`marker-${index}`} marker={marker} radius={config.radius} defaultSize={config.markerSize} onClick={onMarkerClick} onHover={onMarkerHover} />
      ))}
    </group>
  );
}
...
export function Globe3D({ markers = [], config = {}, className, onMarkerClick, onMarkerHover }) {
  ...
  return (
    <div className={cn("relative h-[500px] w-full", className)}>
      ...
      >
        <Suspense fallback={<Html center><span className="text-neutral-400 text-5xl">Loading globe...</span></Html>}>
